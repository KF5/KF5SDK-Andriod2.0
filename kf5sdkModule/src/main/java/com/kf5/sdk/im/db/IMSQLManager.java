...
public class IMSQLManager extends SQLManager {
    ...
    private static volatile IMSQLManager instance;
    ...
    private static ContentValues bindContentValues(IMMessage iMMessage) {
        ...
        contentValues.put(DataBaseColumn.MESSAGE_ID, iMMessage.getMessageId());
        ...
        if (upload != null) {
            ...
            contentValues.put(DataBaseColumn.IMG_WIDTH, upload.getWidth());
            contentValues.put(DataBaseColumn.IMG_HEIGHT, upload.getHeight());
            contentValues.put(DataBaseColumn.VOICE_DURATION, upload.getVoiceDuration());
        }
    ...
    public static List<IMMessage> getPageMessages(Context context, long count) {
        ...
        Cursor cursor = getInstance(context).openSqlDB().rawQuery(sql, new String[]{String.valueOf(offCount), String.valueOf(offCount > 0 ? limit : count)});
...
//                    contentValues.put(DataBaseColumn.MARK, iMMessage.getTimeStamp());
     ...
     * @param height
     */
    public static void updateImageSizeByMessageId(Context context, int messageId, int width, int height) {
        if (context == null || messageId <= 0 || width <= 0 || height <= 0) {
            return;
        }
        LogUtil.printf("更新图片尺寸");
        ContentValues contentValues = new ContentValues();
        contentValues.put(DataBaseColumn.IMG_WIDTH, width);
        contentValues.put(DataBaseColumn.IMG_HEIGHT, height);
        getInstance(context).openSqlDB().update(DataBaseHelper.DB_TABLE, contentValues, DataBaseColumn.MESSAGE_ID + " = ?", new String[]{String.valueOf(messageId)});
    }
     ...
     * @param voiceDuration
     */
    public static void updateVoiceDurationByMessageId(Context context, int messageId, int voiceDuration) {
        if (context == null || voiceDuration <= 0 || messageId <= 0) {
            return;
        }
        LogUtil.printf("更新语音时长");
        ContentValues contentValues = new ContentValues();
        contentValues.put(DataBaseColumn.VOICE_DURATION, voiceDuration);
        getInstance(context).openSqlDB().update(DataBaseHelper.DB_TABLE, contentValues, DataBaseColumn.MESSAGE_ID + " = ?", new String[]{String.valueOf(messageId)});
    }
    ...
    private static List<IMMessage> convertToStorage(Cursor cursor) {
        ...
        try {
            for (int i = 0; i < count; i++) {
                ...
                iMMessage.setMessageId(cursor.getInt(cursor.getColumnIndexOrThrow(DataBaseColumn.MESSAGE_ID)));
...
//                        iMMessage.setStatus(Status.SENDING);
                ...
                if (TextUtils.equals(Field.CHAT_UPLOAD, type)) {
                    ...
                    upload.setWidth(cursor.getInt(cursor.getColumnIndexOrThrow(DataBaseColumn.IMG_WIDTH)));
                    upload.setHeight(cursor.getInt(cursor.getColumnIndexOrThrow(DataBaseColumn.IMG_HEIGHT)));
                    upload.setVoiceDuration(cursor.getInt(cursor.getColumnIndexOrThrow(DataBaseColumn.VOICE_DURATION)));
