...
public class CheckRadioView extends AppCompatImageView {
    ...
    private void init() {
        mSelectedColor = ResourcesCompat.getColor(
                getResources(), R.color.kf5_album_zhihu_item_checkCircle_backgroundColor,
                getContext().getTheme());
        mUnSelectUdColor = ResourcesCompat.getColor(
                getResources(), R.color.kf5_album_zhihu_check_original_radio_disable,
                getContext().getTheme());
        setChecked(false);
    }
    ...
    public void setChecked(boolean enable) {
        if (enable) {
            setImageResource(R.drawable.kf5_album_ic_preview_radio_on);
            mDrawable = getDrawable();
            mDrawable.setColorFilter(mSelectedColor, PorterDuff.Mode.SRC_IN);
        } else {
            setImageResource(R.drawable.kf5_album_ic_preview_radio_off);
            mDrawable = getDrawable();
